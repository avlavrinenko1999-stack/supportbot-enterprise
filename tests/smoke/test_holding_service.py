...
@pytest.mark.asyncio
async def test_add_company_to_holding() -> None:
    ...
    assert company.organization_id == 10
    assert company.holding_id == 20

    event = session.add.call_args.args[0]

    assert isinstance(event, HoldingAuditEvent)
    assert event.event_type == "holding_company_added"
    assert event.payload == {
        "company_id": 30,
        "company_name": "Компания Один",
        "old_organization_id": None,
        "new_organization_id": 10,
    }
...
@pytest.mark.asyncio
async def test_add_company_preserves_matching_organization() -> None:
    from app.models.company import Company
    ...
    await HoldingService(session).add_company(
        20,
        30,
    )
...
@pytest.mark.asyncio
async def test_add_company_from_other_organization_is_rejected() -> None:
    from app.models.company import Company
    ...
    assert company.holding_id is None
    session.commit.assert_not_awaited()
    session.add.assert_not_called()


@pytest.mark.asyncio
async def test_add_company_from_other_holding_is_rejected() -> None:
    from app.models.company import Company
...
@pytest.mark.asyncio
async def test_add_existing_company_is_noop() -> None:
    from app.models.company import Company
...
@pytest.mark.asyncio
async def test_remove_company_from_holding() -> None:
    from app.models.company import Company
    ...
    assert isinstance(event, HoldingAuditEvent)
    assert event.event_type == "holding_company_removed"
    assert event.payload == {
        "company_id": 30,
        "company_name": "Компания Один",
        "organization_id": 10,
...
@pytest.mark.asyncio
async def test_remove_company_from_other_holding_is_rejected() -> None:
    from app.models.company import Company
    ...
    with pytest.raises(
        ValueError,
        match="не входит в этот холдинг",
    ):
        await HoldingService(session).remove_company(
            20,
            30,
        )

    assert company.holding_id == 21
    session.commit.assert_not_awaited()


@pytest.mark.asyncio
async def test_remove_unassigned_company_is_noop() -> None:
    from app.models.company import Company
...
@pytest.mark.asyncio
async def test_add_company_rolls_back_on_audit_error() -> None:
    from app.models.company import Company
