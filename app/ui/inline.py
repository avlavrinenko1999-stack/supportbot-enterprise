...
def confirm_menu(
    *,
    ...
    keyboard = [[button(yes_text, yes_callback), button(no_text, no_callback)]]

    if back_callback:
        keyboard.append([button("⬅️ Назад", back_callback)])

    return InlineKeyboardMarkup(inline_keyboard=keyboard)


def back_menu(*, text: str, callback_data: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[[button(text, callback_data)]]
    )
