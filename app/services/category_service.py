...
class CategoryService:
    ...
    async def list_active_categories(self, company_id: int) -> list[Category]:
        return list(
    ...
    async def list_archived_categories(self, company_id: int) -> list[Category]:
        return list(
            await self.session.scalars(
                select(Category)
                .where(
    ...
    async def get_category_stats(self, category_id: int) -> CategoryWithStats:
    ...
    async def create_category(
        ...
        *,
        ...
        if len(clean_name) < 2:
            raise ValueError("Название категории слишком короткое.")
        ...
        if parent_id is not None:
            ...
            if parent.company_id != company_id:
                raise ValueError("Родительская категория принадлежит другой компании.")

            if parent.is_archived:
                raise ValueError("Нельзя создать подкатегорию внутри архивной категории.")
        ...
        if duplicate is not None:
            raise ValueError("Активная категория с таким названием уже существует.")
    ...
    async def restore_category(self, category_id: int) -> Category:
        ...
        if duplicate is not None:
            raise ValueError(
                "Нельзя восстановить категорию: активная категория с таким названием уже существует."
            )
