...
class CompanyCreationService(BaseService):
    """
    Создание legacy-компаний и заполнение их реквизитов
    данными DaData.
    """
    ...
    async def create_company_from_legal_data(
        self,
        data: DadataCompany,
    ) -> Company:
    ...
    async def find_duplicate_by_legal_data(
        ...
        *,
        ...
        if target_names:
            companies = list(
                await self.session.scalars(select(Company).order_by(Company.id))
            )
